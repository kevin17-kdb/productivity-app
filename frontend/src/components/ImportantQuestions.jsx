...
import { Star, Sparkles, AlertCircle, RefreshCw } from "lucide-react";
...
function ImportantQuestions() {
    ...
    const generate = async () => {
        ...
        setError("");
        try {
            const res = await api.post("/important-questions", { subject });
            setQuestions(res.data.questions);
        } catch (err) {
            console.error("Important Questions generation error:", err);
            setError(err.response?.data?.detail || err.message || "Failed to generate important questions.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div style={{ padding: "36px 40px", maxWidth: "900px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "32px" }}>
                ...
                <div>
                    ...
                    <p style={{ fontSize: "13px", color: "rgba(180,172,230,0.5)", marginTop: "3px" }}>
                        Extract high-priority exam questions from your notes
                    </p>
                </div>
            </div>
            ...
            <button
                ...
                style={{
                    ...
                    transition: "all 0.2s",
                    background: loading ? "rgba(251,113,133,0.15)" : "linear-gradient(135deg,#fb7185,#e11d48)",
                    color: loading ? "rgba(255,255,255,0.4)" : "#fff",
                    boxShadow: loading ? "none" : "0 6px 20px rgba(251,113,133,0.38)",
                }}
            >
                <Sparkles size={16} /> {loading ? "Generating…" : "Generate Questions"}
            </button>
            ...
            {error && (
                ...
                >
                    <div style={{ display: "flex", alignItems: "flex-start", gap: "12px" }}>
                        ...
                        <div>
                            <p style={{ fontWeight: 600, color: "#fda4a4", fontSize: "15px" }}>Generation Failed</p>
            ...
            {questions && (
                <>
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "28px", marginBottom: "14px" }}>
                        <CopyButton content={questions} />
                        <DownloadButton title={`${subject} Important Questions`} content={questions} />
                    </div>
                    <div
                        style={{
                            borderRadius: "16px",
